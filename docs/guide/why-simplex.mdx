---
title: Why SimpleX
description: Why SimpleX gives OpenClaw an invitation-first channel without phone numbers or hosted bot accounts.
---
...
This plugin takes a different shape. With SimpleX, the agent becomes reachable when you hand someone a link. There is no bot registry, no phone-number binding, and no hosted bot account to provision before the first conversation starts.
...
<Columns cols={2}>
  <Card title="Operator-controlled runtime" icon="server">
    Run <code>simplex-chat</code> yourself, choose whether relays are public or self-hosted, and keep OpenClaw policy enforcement local.
  </Card>
  <Card title="Usable in production workflows" icon="message-circle">
    Pairing approval, allowlists, invite helpers, media support, and channel status all exist in the plugin, not in ad hoc glue code.
  </Card>
</Columns>
