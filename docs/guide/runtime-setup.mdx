...
Use per-account config when you want separate SimpleX identities or separate runtime ports for different OpenClaw channel accounts. Each account can point at its own <code>simplex-chat</code> WebSocket endpoint, and that endpoint must already be running.
