...
> Breaking in `1.0.0`: if you are upgrading an older `simplex` install, run `openclaw simplex migrate` before switching commands and config to `openclaw-simplex`. See [Migration](/guide/migration).
...
If `simplex-chat` is not running at the configured endpoint, OpenClaw will mark the channel disconnected and record the connection error in channel status.

If you are upgrading an older `0.x` install that used `simplex` as the plugin/channel id, run:

```bash
openclaw simplex migrate
```

Use `--dry-run` to preview changes. Full upgrade notes are in [Migration](/guide/migration).
