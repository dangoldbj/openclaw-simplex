...
- managed mode was removed; OpenClaw no longer starts `simplex-chat` for you
...
<Steps>
  <Step title="Start the external SimpleX runtime">
    Managed mode is gone. Run <code>simplex-chat</code> yourself and expose the WebSocket endpoint the channel should use:
