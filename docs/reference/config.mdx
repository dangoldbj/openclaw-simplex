...
| `mentionPatterns.mode` | Where agent name patterns count as a mention: `allow` limits pattern matching to the `allowIn` groups; `deny` disables it in the `denyIn` groups. |
| `mentionPatterns.allowIn` | SimpleX group ids where agent name patterns count as a mention (used with `mode: "allow"`). |
| `mentionPatterns.denyIn` | SimpleX group ids where agent name patterns must not count as a mention (used with `mode: "deny"`). |
...
| `draftChunk.minChars` | Minimum characters before the live draft updates. |
| `draftChunk.maxChars` | Maximum characters shown in a single live draft update. The final reply is never truncated. |
| `draftChunk.breakPreference` | Where a live draft update trims back to: `paragraph` \| `newline` \| `sentence`. |
...
<Note>
  <code>draftChunk</code> is OpenClaw's standard live-draft chunking shape and is optional. When set, it takes precedence over <code>streaming.minChars</code> and <code>streaming.wordBoundary</code>; when unset, those legacy settings and their defaults are unchanged. It only shapes the live in-progress draft — the final reply is always delivered in full and never truncated.
...
<Note>
  <code>mentionPatterns</code> scopes where the agent's name patterns count as a mention, so a bot can stay quiet in some groups and respond to name mentions in others. It refines mention handling only; <code>groupPolicy</code>, <code>groupAllowFrom</code>, and per-group <code>requireMention</code> still decide whether a group is handled at all.
...
- <code>draftChunk.minChars</code>, <code>draftChunk.maxChars</code>, and <code>draftChunk.breakPreference</code>
- <code>messageTtlSeconds</code> and <code>filePolicy</code>
- OpenClaw policy and UX settings such as <code>dmPolicy</code>, <code>allowFrom</code>, <code>groupPolicy</code>, <code>groupAllowFrom</code>, <code>mentionPatterns</code>, <code>contextVisibility</code>, <code>actions</code>, and <code>reactionLevel</code>
